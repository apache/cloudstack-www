    ...
    storage: {
      ...
      integrations: [
        ...
        },
        {
          key: "ceph",
          title: "Ceph",
          description: "Ceph is an open-source, software-defined storage platform that provides scalable and resilient block storage through its RADOS Block Device (RBD) interface. Designed for high-performance and fault-tolerant environments, Ceph distributes data across multiple nodes, ensuring redundancy and eliminating single points of failure. Its architecture supports features like thin provisioning, snapshots, and cloning, making it suitable for dynamic and demanding cloud infrastructures.<br/>Apache CloudStack integrates seamlessly with Ceph RBD, allowing administrators to configure Ceph as primary storage within CloudStack deployments. This integration enables the provisioning and management of virtual machine (VM) volumes directly on Ceph, leveraging its distributed storage capabilities. Administrators can define storage pools in Ceph, create corresponding disk offerings in CloudStack, and manage VM disk operations such as creation, deletion, and snapshotting through CloudStack's interface.<br/>By combining Ceph's robust storage features with CloudStack's orchestration capabilities, organizations can deploy a flexible and resilient cloud infrastructure that meets diverse workload requirements.",
        ...
        },
        {
          key: "powerflex",
          title: "PowerFlex",
          description: "Dell PowerFlex is a software-defined storage (SDS) platform that delivers high-performance, scalable, and resilient block storage by aggregating local storage resources across standard x86 servers. Designed for modern data centers, PowerFlex transforms direct-attached storage into a unified, shared storage pool, providing flexibility and efficiency for various workloads. <br/>Apache CloudStack integrates natively with Dell PowerFlex, enabling it to function as managed primary storage within CloudStack environments utilizing the KVM hypervisor. This integration allows administrators to configure PowerFlex storage pools at the zone or cluster level through CloudStack's UI or API. Key features supported include dynamic volume provisioning, disk-only snapshots, thin provisioning, per-volume Quality of Service (QoS) parameters such as bandwidth and IOPS limits, and live volume migration within and across PowerFlex storage pools.<br/>Recent enhancements to the PowerFlex plugin in CloudStack versions 4.18 and above include support for volume encryption, improved management of Storage Data Client (SDC) connections, and dynamic control of client connections on KVM hosts. These features enhance data security, performance, and operational efficiency.<br/>By leveraging Dell PowerFlex's robust storage capabilities within Apache CloudStack, organizations can build flexible, high-performance cloud infrastructures that meet diverse workload requirements.",
